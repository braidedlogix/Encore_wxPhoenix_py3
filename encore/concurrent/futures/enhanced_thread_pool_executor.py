...
class _EnhancedThreadPoolExecutor(_base.Executor):
...
class EnhancedThreadPoolExecutor(_EnhancedThreadPoolExecutor):

    def __init__(self, max_workers, initializer=None, uninitializer=None,
                 name=None):
        self._custom_initializer = initializer
        self._custom_uninitializer = uninitializer
        self._profilers = {}
        super(EnhancedThreadPoolExecutor, self).__init__(
            max_workers, self._initializer, self._uninitializer,
            name)

    def _initializer(self):
        if self._custom_initializer is not None:
            self._custom_initializer()
        thread = threading.current_thread()
        self._profilers[thread] = profiler = cProfile.Profile()
        profiler.enable()

    def _uninitializer(self):
        thread = threading.current_thread()
        profiler = self._profilers.pop(thread)
        profiler.disable()
        profiler.dump_stats('{}-{}.pstats'.format(thread.name, thread.ident))
        if self._custom_uninitializer is not None:
            self._custom_uninitializer()